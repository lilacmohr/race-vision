...
## Phase 9: Project 2 — Decorated Apparel (Job-Relevant)

Apply everything you learned to a new domain:

- Collect garment images (t-shirts, hoodies, polos) from product photography
- Run the same EDA process — check for source homogeneity before labeling
- Define detection classes relevant to decorated apparel:
  - `left_chest` / `full_chest` / `back` / `sleeve` (print zone regions)
  - `embroidery` / `screen_print` / `dtg_print` (existing decoration types)
- Fine-tune a new YOLO model on this dataset
- This directly mirrors real work you'll do professionally
...
| Project 2 (apparel) | repeat of above, faster |
